...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!produccion) {
      return NextResponse.json(
        {
          message: "Producción académica no encontrada."
        }, {
          status: 404
        }
      );
...
export async function PUT(request, { params }) {
  try {
    ...
    if (produccion.pd_id_libro) {
      ...
      const { lib_autores, lib_estado_actual, lib_pagina, lib_pais, lib_edicion, lib_isbn, lib_titulo_libro, lib_tipo_libro, lib_editorial, lib_tiraje, lib_fecha_publicacion } = body;
      await conn.query(
        `
        UPDATE libros
        SET
        ...
        WHERE
          lib_id = ?
        `,
        [lib_autores, lib_estado_actual, lib_pagina, lib_pais, lib_edicion, lib_isbn, lib_titulo_libro, lib_tipo_libro, lib_editorial, lib_tiraje, lib_fecha_publicacion, produccion.pd_id_libro]
      );
    }
